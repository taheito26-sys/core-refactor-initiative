...
-- already mirrored.
--
-- Note: an earlier migration (20260421120000) attempted to redefine this
-- function with a 34-param signature (cash-account links) that put a
-- required parameter after defaulted ones -- invalid in Postgres, so that
-- CREATE FUNCTION always failed and never actually replaced anything. The
-- function actually live today is still the 29-param version from
-- 20260421100000. This migration builds on that real, working signature.
