-- CRITICAL: Remove duplicate chat_send_message function signatures
-- This fixes PGRST203 "Could not choose the best candidate function" error
-- that prevents all message sending in chat.

-- Drop ALL versions of chat_send_message to clear the conflict
DROP FUNCTION IF EXISTS public.chat_send_message(UUID, TEXT, public.chat_message_type, JSONB, UUID, TEXT, TIMESTAMPTZ, BOOLEAN, TEXT);
...
-- Recreate the ONLY correct version with all parameters including _attachment_id
