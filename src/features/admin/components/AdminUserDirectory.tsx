          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            ...
            <tbody>
              {users.map((u, idx) => (
                ...
                >
                  <td style={{ padding: '10px 12px', fontWeight: 500, color: 'var(--tracker-text)', whiteSpace: 'nowrap' }}>{u.email}</td>
                  <td style={{ padding: '10px 12px', color: 'var(--tracker-muted)' }}>{u.display_name ?? '—'}</td>
                  <td style={{ padding: '10px 12px', fontFamily: 'var(--lt-font-mono, monospace)', color: 'var(--tracker-muted)', fontSize: 11 }}>{u.merchant_id ?? '—'}</td>
                  <td style={{ padding: '10px 12px' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                      <span style={{ width: 6, height: 6, borderRadius: '50%', background: STATUS_DOT[u.status] ?? 'var(--tracker-muted)', flexShrink: 0 }} />
                      <span style={{ color: 'var(--tracker-muted)', textTransform: 'capitalize' }}>{u.status}</span>
                    </span>
                  </td>
