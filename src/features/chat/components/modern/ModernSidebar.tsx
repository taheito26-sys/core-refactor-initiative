...
import { Hash, Lock, Settings, Search, Edit, Bell } from 'lucide-react';
...
export function ModernSidebar({ conversations }: Props) {
              ...
              {grouped[lane].map(room => {
                ...
                return (
                  <button 
                    key={room.id}
                    style={{ 
                      width: '100%', background: room.id === 'room-secure-deal-1' ? '#1e293b' : 'transparent', // Hacky mock active state for demo parity
                      border: 'none', padding: '8px 12px', borderRadius: 6, color: room.id === 'room-secure-deal-1' ? '#fff' : '#cbd5e1',
                      display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', textAlign: 'left',
                      fontWeight: room.id === 'room-secure-deal-1' ? 600 : 500
                    }}
                  >
