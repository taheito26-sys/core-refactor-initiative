...
export function LiquidityTab({ onOpenRelationship, onOpenChat, onOpenDeal }: LiquidityTabProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success('Liquidity publishing preferences updated');
    } catch (error: any) {
      toast.error(error?.message || 'Failed to publish liquidity settings');
    }
  };
  ...
  const renderAmount = (side: {
    ...
    if (!side.enabled) return <span style={{ color: 'var(--muted)' }}>Hidden</span>;
    if (side.mode === 'status') return <span className={`pill ${side.status === 'available' ? 'good' : side.status === 'limited' ? 'warn' : 'bad'}`}>{side.status}</span>;
  ...
  if (isLoading || !myProfile || !draft) {
    return <div className="empty"><div className="empty-t">Loading liquidity workspace…</div></div>;
  }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div className="kpi-band">
        <div className="kpi-band-title">Liquidity Market Overview</div>
        <div className="kpi-band-cols" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
          <div><div className="kpi-period">Shared Cash</div><div className="kpi-cell-val">{fmtU(overview.totalCashAvailable)}</div></div>
          <div><div className="kpi-period">Shared USDT</div><div className="kpi-cell-val">{fmtU(overview.totalUsdtAvailable)}</div></div>
          <div><div className="kpi-period">Active Merchants</div><div className="kpi-cell-val">{overview.activeMerchantsCount}</div></div>
          <div><div className="kpi-period">Stale Postings</div><div className="kpi-cell-val" style={{ color: overview.staleCount > 0 ? 'var(--warn)' : 'var(--muted)' }}>{overview.staleCount}</div></div>
          <div><div className="kpi-period">Freshness</div><div className="kpi-cell-val" style={{ fontSize: 11 }}>{overview.mostRecentUpdate ? new Date(overview.mostRecentUpdate).toLocaleString() : 'No updates'}</div></div>
        </div>
      </div>

      <div className="panel" style={{ padding: 10 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          <div style={{ fontSize: 12, fontWeight: 700 }}>Liquidity Board</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <select value={filters.side} onChange={(e) => setFilters((s) => ({ ...s, side: e.target.value as LiquidityFilters['side'] }))}>
              <option value="both">Cash + USDT</option>
              <option value="cash">Cash only</option>
              <option value="usdt">USDT only</option>
            </select>
            <input className="inputBox" style={{ width: 120 }} value={String(filters.minAmount)} onChange={(e) => setFilters((s) => ({ ...s, minAmount: Number(e.target.value) || 0 }))} placeholder="Min amount" />
            ...
            <select value={filters.updatedRecentlyHours ?? ''} onChange={(e) => setFilters((s) => ({ ...s, updatedRecentlyHours: e.target.value ? Number(e.target.value) : null }))}>
              ...
              <option value="4">Updated ≤ 4h</option>
              <option value="24">Updated ≤ 24h</option>
              <option value="72">Updated ≤ 72h</option>
            </select>
          </div>
        </div>

        {filtered.length === 0 ? (
          <div className="empty"><div className="empty-t">No liquidity postings match your filters.</div></div>
        ) : (
                    ...
                    <tr key={entry.merchantId}>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          <span className="pill">Cash: {entry.cash.mode}</span>
                          <span className="pill">USDT: {entry.usdt.mode}</span>
                          {entry.isStale && <span className="pill warn">stale</span>}
                        </div>
                      </td>
                      <td className="mono" style={{ fontSize: 10 }}>{new Date(entry.updatedAt).toLocaleString()}</td>
                      <td>
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          <button className="rowBtn" disabled={!actions.workspacePath} onClick={() => actions.workspacePath && onOpenRelationship(entry.relationshipId!)}>Open workspace</button>
                          <button className="rowBtn" disabled={!actions.chatPath} onClick={() => actions.chatPath && onOpenChat(entry.relationshipId!)}>Open chat</button>
                          <button className="rowBtn" disabled={!actions.dealPath} onClick={() => actions.dealPath && onOpenDeal(entry.relationshipId!)}>Create deal</button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      <div className="panel" style={{ padding: 10 }}>
        <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 8 }}>Match Counterparties</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <select value={matchSide} onChange={(e) => setMatchSide(e.target.value as 'cash' | 'usdt')}>
            <option value="cash">Need cash</option>
            <option value="usdt">Need USDT</option>
          </select>
          <input className="inputBox" style={{ width: 140 }} value={matchAmount} onChange={(e) => setMatchAmount(e.target.value)} placeholder="Requested amount" />
          <div style={{ fontSize: 10, color: 'var(--muted)' }}>Sorted by sufficiency, relationship, freshness.</div>
        </div>
          ...
          {ranked.slice(0, 5).map((entry, idx) => (
            <div key={`${entry.merchantId}-${idx}`} style={{ border: '1px solid var(--line)', borderRadius: 8, padding: '8px 10px', display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ fontSize: 10, color: 'var(--muted)' }}>{entry.relationshipStatus} · {entry.region || 'region n/a'}</div>
              </div>
              <div>{matchSide === 'cash' ? renderAmount(entry.cash) : renderAmount(entry.usdt)}</div>
            </div>
          ))}
          {ranked.length === 0 && <div className="empty"><div className="empty-s">No counterparties currently available.</div></div>}
        </div>
      </div>

      <div className="panel" style={{ padding: 10 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div style={{ fontSize: 12, fontWeight: 700 }}>My Liquidity</div>
          <span className="pill">Internal cash basis: {fmtU(internal?.cashAvailable || 0)}</span>
        </div>
        ...
        <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <label style={{ fontSize: 10 }}><input type="checkbox" checked={draft.autoSyncEnabled} onChange={(e) => setDraft((s) => s ? ({ ...s, autoSyncEnabled: e.target.checked }) : s)} /> Auto-sync from internal basis</label>
          ...
          <button className="btn" onClick={save} disabled={isSaving}>{isSaving ? 'Saving…' : 'Publish liquidity'}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ border: '1px solid var(--line)', borderRadius: 8, padding: 10, display: 'grid', gap: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <label style={{ fontSize: 10 }}>
          <input type="checkbox" checked={props.enabled} onChange={(e) => props.onEnabledChange(e.target.checked)} /> Publish
        </label>
      </div>
      ...
      {props.mode === 'range' && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <label style={{ fontSize: 10 }}>Min <input className="inputBox" value={String(props.rangeMin ?? '')} onChange={(e) => props.onRangeMinChange(e.target.value ? Number(e.target.value) : null)} style={{ marginLeft: 4, width: 90 }} /></label>
          <label style={{ fontSize: 10 }}>Max <input className="inputBox" value={String(props.rangeMax ?? '')} onChange={(e) => props.onRangeMaxChange(e.target.value ? Number(e.target.value) : null)} style={{ marginLeft: 4, width: 90 }} /></label>
        </div>
      )}
