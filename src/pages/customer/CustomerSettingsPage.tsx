  ...
  return (
    <div className="space-y-5">
          ...
          {[
            ...
            <div key={label}>
              ...
              <input value={value} onChange={e => set(e.target.value)} type={type} placeholder={placeholder}
                className="h-11 w-full rounded-xl border border-border/50 bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
            </div>
          ))}
          <div>
      ...
      <div className="rounded-2xl border border-border/50 bg-card px-4 py-3 space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{L('Account', 'الحساب')}</p>
        ...
        <div className="flex justify-between text-sm">
          ...
          <span>{customerProfile?.preferred_currency ?? 'USDT'}</span>
        </div>
      </div>

      <button onClick={() => logout()}
        className="flex w-full items-center justify-center gap-2 rounded-2xl border border-destructive/30 bg-destructive/5 py-3 text-sm font-semibold text-destructive hover:bg-destructive/10 transition-colors">
