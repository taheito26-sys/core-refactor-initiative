                  ...
                  <div key={order.id} className="overflow-hidden rounded-2xl border border-border/60 bg-card">
                    <div className="p-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-baseline gap-1.5">
                              <span className="text-2xl font-bold">{formatCustomerNumber(order.amount, lang, 2)}</span>
                              <span className="text-xs font-semibold text-muted-foreground">{order.send_currency}</span>
                            </div>
                          </div>
                          <div className="space-y-1">
                            <div className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                            ...
                            <div className="flex items-baseline gap-1.5">
                              ...
                              <span className="text-xs font-semibold text-muted-foreground">{order.receive_currency}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        {/* FX Rate & Date Row */}
                        {order.fx_rate && (
                          <div className="grid grid-cols-2 gap-3 rounded-lg bg-muted/30 px-3 py-2">
                            <div className="space-y-0.5">
                              <div className="text-[10px] font-medium uppercase text-muted-foreground">{L('FX Rate', 'سعر الصرف')}</div>
                              <div className="text-sm font-semibold">1 {order.send_currency} = {formatCustomerNumber(order.fx_rate, lang, 4)} {order.receive_currency}</div>
                            </div>
                            <div className="space-y-0.5 text-right">
                              <div className="text-[10px] font-medium uppercase text-muted-foreground">{L('Date', 'التاريخ')}</div>
                              <div className="text-sm font-semibold">{new Date(order.created_at).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US')}</div>
                            </div>
                          </div>
                        )}
