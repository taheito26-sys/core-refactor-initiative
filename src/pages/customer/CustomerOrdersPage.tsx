...
function LinkCashModal({ orderId, egpAmount, receiveCurrency, lang, onClose }: {
  ...
  const createAccMutation = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase.from('cash_accounts').insert({ id: newId, user_id: userId, name: newName.trim(), type: newType, currency: newCurrency, status: 'active', created_at: Date.now() }).select().single();
    ...
    <div className="space-y-6 pb-16">
      ...
      {linkingOrder && (
        <LinkCashModal
          ...
          egpAmount={deliveredEgpMap.get(linkingOrder.id) ?? (linkingOrder.fx_rate ? Math.round(linkingOrder.amount * linkingOrder.fx_rate) : 0)}
