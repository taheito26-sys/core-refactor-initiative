...
export default function SettingsPage() {
  ...
  const clearAll = async () => { if (!confirm(t.lang === 'ar' ? '⚠ مسح جميع البيانات؟' : '⚠ Clear ALL data?')) return; clearTrackerStorage(localStorage); activateTrackerClearBarrier(localStorage); markTrackerClearInProgress(); const empty = { batches: [], trades: [], customers: [], suppliers: [], cashQAR: 0, cashOwner: '', cashHistory: [], cashAccounts: [], cashLedger: [], currency: 'QAR', range: '7d', settings: { lowStockThreshold: 5000, priceAlertThreshold: 2 }, cal: { year: new Date().getFullYear(), month: new Date().getMonth(), selectedDay: null } }; void saveTrackerStateNow(empty as unknown as TrackerState, { replaceExisting: true, preserveDataCleared: true, allowDuringClear: true }); toast.success('Data cleared — reloading…'); setTimeout(() => window.location.reload(), 500); };
